...
Usage:
uv run scripts/holdout_agent_experiment.py --dataset_file <dataset_file>
--feature_type <feature_type> --k_holdout <k_holdout> --output_dir <output_dir>
--seed <seed>
"""
...
def main():
    ...
    confusion_matrix_dir = os.path.join(args.output_dir, "confusion_matrices")
    ...
    for idx, combination in tqdm(
        ...
        try:
            ...
            matrix_labels = op["cm_labels"]
            matrix_file = os.path.join(confusion_matrix_dir, f"holdout_{tag}.csv")
            pd.DataFrame(
                op["confusion_matrix"],
                index=pd.Index(matrix_labels, name="true"),
                columns=pd.Index(matrix_labels, name="predicted"),
            ).to_csv(matrix_file)

            logging.info(f"Unseen samples absorbed into: {op['absorbed_into']}")
    ...
    logging.info(f"wrote confusion matrices to {confusion_matrix_dir}")
    logging.info(
